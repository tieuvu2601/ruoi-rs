...
import com.banvien.portal.vms.webapp.servlet.fck.CKUploadResponse;
...
public class Dispatcher {
	...
	UploadResponse doPost(final HttpServletRequest request) {
		...
		CKUploadResponse uploadResponse = null;
		// check permissions for user actions
		if (!RequestCycleHandler.isFileUploadEnabled(request))
			uploadResponse = CKUploadResponse.getFileUploadDisabledError();
		// check parameters  
		else if (!Command.isValidForPost(context.getCommandStr()))
			uploadResponse = CKUploadResponse.getInvalidCommandError();
		else if (!ResourceType.isValidType(context.getTypeStr()))
			uploadResponse = CKUploadResponse.getInvalidResourceTypeError();
		else if (!UtilsFile.isValidPath(context.getCurrentFolderStr()))
			uploadResponse = CKUploadResponse.getInvalidCurrentFolderError();
		else {
			...
			try {
				String sCKECallback = request.getParameter("CKEditorFuncNum");
				...
				if (type.isDeniedExtension(FilenameUtils.getExtension(fileName)))
					uploadResponse = CKUploadResponse.getInvalidFileTypeError();
				// Secure image check (can't be done if QuickUpload)
					...
					if (sanitizedFileName.equals(FilenameUtils.getName(newFileName)))
						uploadResponse = CKUploadResponse.getOK(fileUrl, sCKECallback);
					else {
						uploadResponse = CKUploadResponse.getFileRenamedWarning(fileUrl, newFileName, sCKECallback);
				...
				uploadResponse = CKUploadResponse.getFileUploadWriteError();
			} catch (IOException e) {
				uploadResponse = CKUploadResponse.getFileUploadWriteError();
