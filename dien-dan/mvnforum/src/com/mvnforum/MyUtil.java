/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/MyUtil.java,v 1.126.2.2 2009/03/10 09:01:00 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.126.2.2 $
 * $Date: 2009/03/10 09:01:00 $
 *
 * ====================================================================
 *
        ...
        try {
            ...
            for (Iterator iterator = categoryBeans.iterator(); iterator.hasNext();) {
                ...
                if (category.getParentCategoryID() == categoryID) {
                    if (canViewAnyForumInCategory(category.getCategoryID(), permission)) {
                        return true;
                    }
                }
            }
...
//          JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(outputStream);
    ...
    public static boolean isRequestFromMobileDevice(GenericRequest request) {
        if (request.isServletRequest()) {
            String userAgent = request.getServletRequest().getHeader("User-Agent").toLowerCase();
