/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/db/jdbc/AttachmentDAOImplJDBC.java,v 1.41 2008/05/30 04:41:29 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.41 $
...
public class AttachmentDAOImplJDBC implements AttachmentDAO {
    ...
    private static final Log log = LogFactory.getLog(AttachmentDAOImplJDBC.class);
    ...
    public AttachmentBean getAttachment(int attachID)
        ...
        sql.append(" FROM " + TABLE_NAME + " attach LEFT JOIN " + PostDAO.TABLE_NAME + " post");
        sql.append(" ON attach.PostID =  post.PostID");
        sql.append(" WHERE AttachID = ?");
        try {
            connection = DBUtils.getConnection();
            statement = connection.prepareStatement(sql.toString());
    ...
    public Collection getAttachmentsOfMember(int memberID)
        throws DatabaseException {
        ...
        Collection retValue = new ArrayList();
        StringBuffer sql = new StringBuffer(512);
        sql.append("SELECT AttachID, attach.PostID, attach.MemberID, AttachFilename, AttachFileSize, AttachMimeType, AttachDesc, AttachCreationIP, AttachCreationDate, AttachModifiedDate, AttachDownloadCount, AttachOption, AttachStatus");
        sql.append(" FROM " + TABLE_NAME + " attach");
        sql.append(" WHERE attach.MemberID = ?");
        try {
            connection = DBUtils.getConnection();
            statement = connection.prepareStatement(sql.toString());
   ...
   public int getNumberOfAttachments_inThread(int threadID)
    ...
    protected static int findAttachID(int postID, int memberID, Timestamp attachCreationDate)
        ...
        if(postID > 0)
        {
            sql.append(" WHERE MemberID = ? AND AttachCreationDate = ? AND PostID = ?");
        }
        else
        {
            sql.append(" WHERE PostID is null AND MemberID = ? AND AttachCreationDate = ?");
        }
        try {
            connection = DBUtils.getConnection();
            statement = connection.prepareStatement(sql.toString());
            ...
            statement.setInt(1, memberID);
            statement.setTimestamp(2, attachCreationDate);
            if(postID > 0)
            {
