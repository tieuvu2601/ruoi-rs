/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/user/PostWebHandler.java,v 1.189.2.2 2008/11/18 05:54:08 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.189.2.2 $
 * $Date: 2008/11/18 05:54:08 $
 *
 * ====================================================================
 *
...
public class PostWebHandler {
    ...
    private static final Log log = LogFactory.getLog(PostWebHandler.class);
    ...
    public void prepareAdd(GenericRequest request, GenericResponse response)
        ...
        CategoryTreeListener treelistener = categoryService.getCategoryTreePath(request, response, forumID, mode, "addnew", replyTopic);
        ...
        CategoryTreeListener listener = categoryService.getManagementCategorySelector(request, response, "addpost");
    ...
    public void processAdd(GenericRequest request, GenericResponse response)
        ...
        BadInputException, ForeignKeyNotFoundException, AuthenticationException, FloodException, InterceptorException, MessagingException, IOException, TemplateException {
        ...
        try {
            FloodControl.ensureNotReachMaximum(MVNForumGlobal.FLOOD_ID_NEW_POST_PER_MEMBER, memberName);
            ...
            Integer maxPosts = new Integer(FloodControl.getActionsPerHour(MVNForumGlobal.FLOOD_ID_NEW_POST_PER_MEMBER));
        ...
        try {
            FloodControl.ensureNotReachMaximum(MVNForumGlobal.FLOOD_ID_NEW_POST_PER_IP, currentIP);
        } catch (FloodException fe) {
            //throw new FloodException("You have reached the maximum number of the post adding actions for this page. Please try this page later. This is to prevent forum from being flooded.");
            Integer maxPosts = new Integer(FloodControl.getActionsPerHour(MVNForumGlobal.FLOOD_ID_NEW_POST_PER_IP));
            String localizedMessage = MVNForumResourceBundle.getString(locale, "mvncore.exception.FloodException.add_post_too_many_times", new Object[] {maxPosts});
            throw new FloodException(localizedMessage);
        }
        ...
        int showVMSThread = GenericParamUtil.getParameterBoolean(request, "ThreadShowVMS") == true ? 1:0;
        ...
        if (parentPostID == 0) {// new thread
            ...
            int threadPriority = GenericParamUtil.getParameterUnsignedInt(request, "ThreadPriority", ThreadBean.THREAD_PRIORITY_NORMAL);

            if ( threadType > ThreadBean.THREAD_TYPE_GLOBAL_ANNOUNCEMENT /* 3 */) {
                String localizedMessage = MVNForumResourceBundle.getString(locale, "mvncore.exception.BadInputException.not_support_thread_type", new Object[] {new Integer(threadType)});
            ...
            threadID = DAOFactory.getThreadDAO().createThread(forumID, memberName, lastPostMemberName,
                                   ...
                                   threadStatus, threadHasPoll, 0/*threadViewCount*/,
                                   0/*threadReplyCount*/, postIcon, threadDuration, threadAttachCount, showVMSThread);
        ...
        // for enterprise
        if (MvnCoreServiceFactory.getMvnCoreService().getEnvironmentService().getForumRunMode() == EnvironmentService.PRODUCT_ENTERPRISE) {
            if (MVNForumConfig.getEnableAttachment() && permission.canAddAttachment(forumID)) {
                request.setAttribute("UploadApplet", Boolean.valueOf(GenericParamUtil.getParameterBoolean(request, "UploadApplet")));
            }
            if (MVNForumConfig.getEnablePoll()) {
                try {
                    ...
                    if ((permission.canAddPoll(forumID)) || 
                        (threadBean.getMemberName().equals(onlineUser.getMemberName()) && permission.canAddOwnThreadPoll(forumID))) {
        ...
        FloodControl.increaseCount(MVNForumGlobal.FLOOD_ID_NEW_POST_PER_IP, currentIP);
        FloodControl.increaseCount(MVNForumGlobal.FLOOD_ID_NEW_POST_PER_MEMBER, memberName);
    ...
    public void addPostSuccessForRender(GenericRequest request, GenericResponse response)
        ...
        String addPostSuccessLabel = MVNForumResourceBundle.getString(locale, "mvnforum.user.addpostsuccess.title");
        CategoryBuilder treebuilder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree categorytree = new CategoryTree(treebuilder);
        CategoryTreeListener treelistener = categoryService.getCategoryTreePath(request, response, forumID, null, null, addPostSuccessLabel);
        categorytree.addCategeoryTreeListener(treelistener);
        request.setAttribute("tree", categorytree.build());
        ...
        if (requestURI.startsWith("/printpost_")) {
            ...
            if (params.get("post") != null) {
                ...
                try {
                    ...
                    String localizedMessage = MVNCoreResourceBundle.getString(locale, "mvncore.exception.BadInputException.cannot_parse", new Object[] {DisableHtmlTagFilter.filter("post"), "int"});
                    throw new BadInputException(localizedMessage);
                }
            } else {
                String localizedMessage = MVNCoreResourceBundle.getString(locale, "mvncore.exception.BadInputException.not_allow_to_be_empty", new Object[] {DisableHtmlTagFilter.filter("post")});
                throw new BadInputException(localizedMessage);
            }
        } else {
        ...
        if ( (postAttachCount > 0) && MVNForumConfig.getEnableAttachment() ) {
            ...
            if (postAttachCount != actualAttachCount) {
                if (actualAttachCount != DAOFactory.getAttachmentDAO().getNumberOfAttachments_inPost(postID)) {
                    ...
                    //throw new AssertionError("AssertionError: Serious error: cannot process Attachment Count in table Attachment");
                }
    ...
    public void prepareEdit(GenericRequest request, GenericResponse response)
        ...
        request.setAttribute("showVMS", new Integer(threadBean.getThreadShowVMS()));
        ...
        CategoryBuilder treebuilder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree categorytree = new CategoryTree(treebuilder);
        CategoryTreeListener treelistener = categoryService.getCategoryTreePath(request, response, forumID, mode, "update", replyTopic);
        categorytree.addCategeoryTreeListener(treelistener);
        request.setAttribute("tree", categorytree.build());
     ...
     * @todo: check if messageTopic could be optional when we reply
    ...
    public void processUpdate(GenericRequest request)
        throws ObjectNotFoundException, BadInputException, DatabaseException, CreateException, ForeignKeyNotFoundException, 
        AuthenticationException, InterceptorException, IOException, MessagingException, TemplateException {
        ...
        if (MVNForumConfig.getEnableEmailToAdminContentWithCensoredWords()) {
            if (beforeValidated.equals(postBody) == false) {
                PostUtil.sendEmailToAdminBecauseCensoredPost(request, postID, forumID, threadID, logonMemberID);
            }
        }
        if (postBean.getParentPostID() == 0) {//edit a top post (thread)
            String threadIcon = postIcon;
            int threadPriority = GenericParamUtil.getParameterUnsignedInt(request, "ThreadPriority", ThreadBean.THREAD_PRIORITY_NORMAL);
            DAOFactory.getThreadDAO().updateTopic_Body_Icon(threadID, postTopic, postBody, threadIcon, threadPriority);
        }
        ...
        DAOFactory.getThreadDAO().updateThreadShowVMS(threadID, showVMSThread);
    ...
    public void updatePostSuccessForRender(GenericRequest request, GenericResponse response)
        ...
        String addPostSuccessLabel = MVNForumResourceBundle.getString(locale, "mvnforum.user.updatepostsuccess.title");
        CategoryBuilder treebuilder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree categorytree = new CategoryTree(treebuilder);
        CategoryTreeListener treelistener = categoryService.getCategoryTreePath(request, response, forumID, null, null, addPostSuccessLabel);
        categorytree.addCategeoryTreeListener(treelistener);
    ...
    public void deleteSuccessForRender(GenericRequest request, GenericResponse response)
        ...
        String addPostSuccessLabel = MVNForumResourceBundle.getString(locale, "mvnforum.user.deletepostsuccess.title");
        CategoryBuilder treebuilder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree categorytree = new CategoryTree(treebuilder);
        CategoryTreeListener treelistener = categoryService.getCategoryTreePath(request, response, forumID, null, null, addPostSuccessLabel);
        categorytree.addCategeoryTreeListener(treelistener);
    ...
    public void prepareViewThread(GenericRequest request, GenericResponse response, String requestURI)
        throws AuthenticationException, DatabaseException, BadInputException, 
        ObjectNotFoundException, MissingURLMapEntryException {
        ...
        if (requestURI.startsWith("/viewthread_") || requestURI.startsWith("/printthread_")) {
            ...
            if (params.get("thread") != null) {
                ...
                try {
                    ...
                    String localizedMessage = MVNCoreResourceBundle.getString(locale, "mvncore.exception.BadInputException.cannot_parse", new Object[] {DisableHtmlTagFilter.filter("thread"), "int"});
                    throw new BadInputException(localizedMessage);
                }
            } else {
                String localizedMessage = MVNCoreResourceBundle.getString(locale, "mvncore.exception.BadInputException.not_allow_to_be_empty", new Object[] {DisableHtmlTagFilter.filter("thread")});
                throw new BadInputException(localizedMessage);
            }
            
            printAll = ((params.get("printall") != null) && (((String)params.get("printall")).equalsIgnoreCase("yes"))) ? true : false;
            lastPage = ((params.get("lastpage") != null) && (((String)params.get("lastpage")).equalsIgnoreCase("yes"))) ? true : false;
        } else {
            threadID = GenericParamUtil.getParameterInt(request, "thread");
            ...
            try {
                if (requestURI.startsWith("/viewthread_") || requestURI.startsWith("/printthread_")) {
                    ...
                    try {
                        ...
                        if (offset < 0) {
                            String localizedMessage = MVNCoreResourceBundle.getString(locale, "mvncore.exception.BadInputException.must_be_unsigned_value", new Object[] {DisableHtmlTagFilter.filter("offset")});
                            throw new BadInputException(localizedMessage);
                        }
                    } catch (NumberFormatException e) {
                        String localizedMessage = MVNCoreResourceBundle.getString(locale, "mvncore.exception.BadInputException.cannot_parse", new Object[] {DisableHtmlTagFilter.filter("thread"), "int"});
        ...
        while(iterator.hasNext()) {
            ...
            if (postBean.getMemberID() != 0 && postBean.getMemberID() != MVNForumConstant.MEMBER_ID_OF_GUEST) {
                ...
                try {
                    ...
                    // do nothing, we accept null member because sometime in LDAP or portal, we cannot control delete user in LDAP
                }
            }
            postBean.setMemberBean(memberBean);
        ...
        //int nextTopic     = DAOFactory.getThreadDAO().getNextEnableThread(forumID, threadID);// can throw AssertionError
        int previousTopic = ThreadCache.getInstance().getPreviousEnableThread(forumID, threadID);// can throw AssertionError
        int nextTopic     = ThreadCache.getInstance().getNextEnableThread(forumID, threadID);// can throw AssertionError
        ...
        PostBean lastpostBean = PostCache.getInstance().getLastEnablePost_inThread(threadBean.getThreadID());
        request.setAttribute("ThreadBean", threadBean);
        request.setAttribute("PostBeans", postBeans);
        ...
        CategoryBuilder treebuilder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree categorytree = new CategoryTree(treebuilder);
        CategoryTreeListener treelistener = categoryService.getCategoryTreePath(request, response, forumID, 0, threadLabel + ": " + MyUtil.filter(threadBean.getThreadTopic(), false/*html*/, true/*emotion*/, false/*mvnCode*/, false/*newLine*/, false/*URL*/));
        categorytree.addCategeoryTreeListener(treelistener);
        request.setAttribute("tree", categorytree.build());

        CategoryBuilder builder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree tree = new CategoryTree(builder);
        CategoryTreeListener listener = categoryService.getManagementCategorySelector(request, response, "listthreads", forumID);
        tree.addCategeoryTreeListener(listener);
        request.setAttribute("Result", tree.build());

        request.setAttribute("thread", new Integer(threadID));

        //updateOnlineUserAction(request, requestURI) must be called after request.setAttribute("thread", new Integer(threadID));
        ...
        if (MVNForumConfig.getEnableOnlineUsers()) {
            OnlineUserUtil.setRequestAttributeOfOnlineActions(request, Action.PAGE_ID_VIEWTHREAD, new Integer(threadID));
        }
    ...
    public void processSearch(GenericRequest request, GenericResponse response)
        throws BadInputException, IOException, DatabaseException, ObjectNotFoundException, AuthenticationException {
        ...
        if (!MVNForumConfig.getEnableSearch()) {
            String localizedMessage = MVNForumResourceBundle.getString(locale, "java.lang.IllegalStateException.search_disabled");
            throw new IllegalStateException(localizedMessage);
        }
        ...
        CategoryBuilder builder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree tree = new CategoryTree(builder);
        CategoryTreeListener listener = categoryService.getManagementCategorySelector(request, response, "search");
        tree.addCategeoryTreeListener(listener);
        request.setAttribute("Result", tree.build());
        ...
        int sort    = GenericParamUtil.getParameterInt(request, "sort", PostSearchQuery.SEARCH_SORT_DEFAULT);
        ...
        if (key.length() > 0) {
            ...
            int scopeInPost = GenericParamUtil.getParameterInt(request, "scopeinpost", PostSearchQuery.SEARCH_ONLY_BODY|PostSearchQuery.SEARCH_ONLY_TITLE);
        ...
        int searchDate        = GenericParamUtil.getParameterUnsignedInt(request, "date", PostSearchQuery.SEARCH_ANY_DATE);
        int searchBeforeAfter = GenericParamUtil.getParameterInt(request, "beforeafter", PostSearchQuery.SEARCH_NEWER);
