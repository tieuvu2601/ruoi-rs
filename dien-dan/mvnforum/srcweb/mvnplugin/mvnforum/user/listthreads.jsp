<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/listthreads.jsp,v 1.204 2008/06/13 09:23:25 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.204 $
...
<table width="95%" align="center">
  <tr class="nav">
    ...
    <fmt:message key="mvnforum.common.forum"/>: <%=currentForumBean.getForumName()%>
    --%>
    <%
  ...
  <tr class="portlet-font">
    ...
    <td align="right" valign="bottom">
      ...
      <% if (MVNForumConfig.getEnableEasyWatching() && onlineUser.isMember()) {
        ...
        <% } else { %>
            <span class="portlet-font"><fmt:message key="mvnforum.user.listthreads.not_watching_forum"/></span> <a class="command" href="<%=urlResolver.encodeURL(request, response, "addwatchprocess?WatchType=" + WatchBean.WATCH_TYPE_DEFAULT + "&amp;WatchSelector=" + WatchBean.SELECT_FORUM_WATCH + "&amp;forum=" + forumID)%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_watch.gif" border="0" alt="<fmt:message key="mvnforum.user.listthreads.watch_forum"/>" title="<fmt:message key="mvnforum.user.listthreads.watch_forum"/>" /></a>
        <% } 
         } %>
      ...
      <a class="command" href="<%=urlResolver.encodeURL(request, response, "addpost?forum=" + forumID)%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_new_thread.gif" border="0" alt="<fmt:message key="mvnforum.user.listthreads.new_thread"/>" title="<fmt:message key="mvnforum.user.listthreads.new_thread"/>" /></a>
      <% }//if can new post%>
    </td>
  </tr>
</table>
...
<table width="95%" align="center">
  <tr>
    ...
    <% if (MVNForumConfig.getEnableNewPost() && (onlineUser.isGuest()||permission.canAddThread(forumID)) && (currentForumBean.getForumStatus()==ForumBean.FORUM_STATUS_DEFAULT) ) { %>
    <td align="right"><a class="command" href="<%=urlResolver.encodeURL(request, response, "addpost?forum=" + forumID)%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_new_thread.gif" border="0" alt="<fmt:message key="mvnforum.user.listthreads.new_thread"/>" title="<fmt:message key="mvnforum.user.listthreads.new_thread"/>" /></a></td>
    <% }//if can new post%>
  </tr>
</table>
