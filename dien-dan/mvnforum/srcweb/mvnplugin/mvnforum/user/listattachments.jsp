<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/listattachments.jsp,v 1.32 2008/06/13 09:23:25 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.32 $
...
function handleGo() {
  var forumID = 0;
  var categoryID = 0;
  if (document.form.obj) {
    if (document.form.obj.value.length > 0) {
      var valueOfParam = document.form.obj.value;
      if (valueOfParam.indexOf("f") != -1) {
        forumID = valueOfParam.substring(1);
      } else if (valueOfParam.indexOf("c") != -1) {
        categoryID = valueOfParam.substring(1);
      }
    }
  }
  ...
  params += "&sort=" + document.form.sort.value;
  params += "&order=" + document.form.order.value;

  gotoPage("listattachments?" + params);
}
//]]>
</script>
...
<mvn:cssrows>
  ...
  <tr class="<mvn:cssrow/>">
    <td><fmt:message key="mvnforum.user.searchattachments.by_description"/></td>
    <td>
      <input type="text" size="60" name="key" onkeyup="initTyper(this);" /> - 
...
%>
<form name="form" action="<%=urlResolver.encodeURL(request, response, "listattachments", URLResolverService.ACTION_URL)%>" <mvn:method/> onsubmit="return false">
<%=urlResolver.generateFormAction(request, response, "listattachments")%>
<table width="95%" align="center">
  <tr class="portlet-font">
    <td>
      <fmt:message key="mvnforum.common.filter_by.category_or_forum"/> &raquo;
