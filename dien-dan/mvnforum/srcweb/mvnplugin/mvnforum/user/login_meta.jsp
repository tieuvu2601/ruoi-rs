<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/login_meta.jsp,v 1.8 2008/06/13 08:32:24 minhnn Exp $
 - $Author: minhnn $
 - $Revision: 1.8 $
 - $Date: 2008/06/13 08:32:24 $
 -
 - ====================================================================
 -
...
$(document).ready(function(){
    ...
    $("#input_login_username input").keypress(function (e) {
        if (e.which == 13 ) {
            SubmitFormLogin();
        }
    });

    $("#input_login_password input").keypress(function (e) {
        if (e.which == 13 ) {
            SubmitFormLogin();
        }
    });
    ...
    <%=urlResolver.generateFormAction(request, response, "loginprocess")%>
      <td>
        ...
        <input type="hidden" name="url" value="<%=DisableHtmlTagFilter.filter(ParamUtil.getParameter(request, "url"))%>" />
          <div id="input_login_username">
                <div class="text"> <fmt:message key="mvnforum.common.member.login_name" /> </div>
                <input type="text" name="MemberName" value="<%=ParamUtil.getAttribute(request, "MemberName")%>" onkeypress="checkEnterLogin(event);" size="10" />
          </div>
          <div id="input_login_password">
              <div class="text"> <fmt:message key="mvnforum.common.member.password"/> </div>
              <input type="password" name="MemberMatkhau" size="10" />
          </div>

          <div class="submit_login">
              <input type="button" name="submitbutton" value="<fmt:message key="mvnforum.common.action.login"/>" onclick="javascript:SubmitFormLogin();" />
          </div>
