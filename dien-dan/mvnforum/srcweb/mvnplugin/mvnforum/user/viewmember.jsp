<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/viewmember.jsp,v 1.102 2008/06/13 09:23:25 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.102 $
...
<mvn:cssrows>
  ...
  <%}%>
  <tr class="<mvn:cssrow/>">
    <td width="25%"><fmt:message key="mvnforum.common.member.id"/></td>
    <td>
    ...
    <%boolean enableInvisible = MVNForumConfig.getEnableInvisibleUsers();
      ...
      <a href="<%=urlResolver.encodeURL(request, response, "addmessage?MessageToList=" + Encoder.encodeURL(memberBean.getMemberName()))%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_pm.gif" border="0" alt="<fmt:message key="mvnforum.user.addmessage.title"/>" title="<fmt:message key="mvnforum.user.addmessage.title"/>"/></a>
      <%}%>
    </td>
  </tr>
  ...
  <%} %>
  <% if (permission.canAdminSystem() && (memberBean.getMemberEmail().equalsIgnoreCase(memberBean.getMemberFirstEmail()) == false) ) {%>
  ...
  <a href="<%=urlResolver.encodeURL(request, response, "sendmail?ToMember=" + memberBean.getMemberName())%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_email.gif" border="0" alt="<fmt:message key="mvnforum.user.sendmail.title"/>" title="<fmt:message key="mvnforum.user.sendmail.title"/>"/></a>
    </td>
  </tr>
  <%} %>
...
<%--
  <tr class="<mvn:cssrow/>">
    ...
    <%if (memberBean.showEmail()) {
      ...
      <%} else {%>
      <a href="mailto:<%=memberBean.getMemberEmail()%>"><%=memberBean.getMemberEmail()%></a>
      <%}// end of online user is not a guest %>
    <%} else if (permission.canAdminSystem()) {// // end of show email%>
      <a href="mailto:<%=memberBean.getMemberEmail()%>"><%=memberBean.getMemberEmail()%></a>
    <%} else {%>
        <fmt:message key="mvnforum.common.member.email.dont_want_to_show"/>
    <%}// end outer if struct %>
      <% if (permission.canAdminSystem() && (memberBean.getMemberEmail().equalsIgnoreCase(memberBean.getMemberFirstEmail()) == false) ) {%>
         (<fmt:message key="mvnforum.common.member.first_email"/>: <a href="mailto:<%=memberBean.getMemberFirstEmail()%>"><%=memberBean.getMemberFirstEmail()%></a>)
      <% } // let to show the first email%>
        <a href="<%=urlResolver.encodeURL(request, response, "sendmail?toID=" + memberBean.getMemberID())%>"><img src="<%=contextPath%>/mvnplugin/mvnforum/images/icon/button_email.gif" border="0" alt="Send email to this user" title="Send email to this user"/></a>
    </td>
  </tr>
  <%} %>
--%>
