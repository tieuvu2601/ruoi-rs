<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/addpost.jsp,v 1.223 2008/06/13 09:23:21 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.223 $
...
if(request.getAttribute("showVMS") != null)
{
    showVMS = ((Integer) request.getAttribute("showVMS"));
}
...
<link href="<%=contextPath%>/mvnplugin/mvnforum/css/showimage.css" rel="stylesheet" type="text/css"/>

<%--<script type="text/javascript" src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery.min.js"></script>--%>
<%--<script type="text/javascript" src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery-ui.min.js"> </script>--%>
...
<%--<script language="JavaScript1.2" src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery.reveal.js"> </script>--%>
<%--<script src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery.form.js"></script>--%>
...
function checkChooseAttachMore() {
  if ((document.getElementById("flag_UploadApplet") != null) && (document.getElementById("flag_UploadApplet").value == "true") &&
    ...
    alert("If you want to attach a file, please cancel \"Attach a screenshot\" function.");
    ...
    alert("If you want to attach a file, please cancel \"Add a Poll\" function.");
    document.mvnform.AttachMore.checked = false;
  }
}
...
function checkChooseUploadApplet() {
  if (document.getElementById("flag_UploadApplet") != null && document.getElementById("flag_UploadApplet").value == "true") {
    if (document.mvnform.AttachMore.checked) {
      alert("If you want to attach a screenshot, please cancel \"Attach a file\" function.");
      document.mvnform.UploadApplet.checked = false;
    } else if ((document.getElementById("flag_AddPoll") != null) && (document.getElementById("flag_AddPoll").value == "true")) {
      if (document.mvnform.AddPoll.checked) {
        alert("If you want to attach a screenshot, please cancel \"Add a Poll\" function.");
        document.mvnform.UploadApplet.checked = false;
      }
    }
  }
...
function checkChooseAddPoll() {
  if (document.getElementById("flag_AddPoll") != null && document.getElementById("flag_AddPoll").value == "true") {
    if (document.mvnform.AttachMore.checked) {
      alert("If you want to add a poll, please cancel \"Attach a file\" function.");
      ...
      if (document.mvnform.UploadApplet.checked) {
        alert("If you want to add a poll, please cancel \"Attach a screenshot\" function.");
        document.mvnform.AddPoll.checked = false;
      }
    }
  }
...
<!--jQuery-->
<script src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery-1.8.3.js"></script>
<script src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery.reveal.js"></script>
<script src="<%=contextPath%>/mvnplugin/mvnforum/js/jquery.form.js"></script>

<script type="text/javascript">
	$(document).ready(function() {
		$('#uploadImage').click(function(e) { // Button which will activate our modal
...
<mvn:cssrows>
  ...
  <tr class="<mvn:cssrow/>">
    <td width="170" nowrap="nowrap"><fmt:message key="mvnforum.common.post.topic"/> <span class="requiredfield">*</span></td>
      ...
      <table cellspacing="0" cellpadding="0" class="noborder">
        <tr>
          <td nowrap="nowrap">
            ...
            <input class="liteoption" id="uploadImage" value="<fmt:message key="mvnforum.user.addpost.button.keycaps.imageupload"/>" type="button" />
  ...
  <tr class="<mvn:cssrow/>">
    ...
    <td valign="top"><textarea <%if (MVNForumConfig.getEnableMaximizedEditArea()) {%> style="width:100%" <%} else {%> cols="70" <%}%> rows="18" id="message" name="message" tabindex="2" onkeyup="initTyper(this);storeCaret(this);" onselect="storeCaret(this);" onclick="storeCaret(this);" onmouseup="storeCaret(this);" onchange="storeCaret(this);"><%=postBody%></textarea></td>
  </tr>
    ...
    <td valign="top">
      ...
      <% } %>
        ...
        if ( (action.equals("addnew") && parentPostID == 0) || (action.equals("update") && postToEdit.getParentPostID() == 0) ) { %>
            ...
            <input type="checkbox" name="ThreadShowVMS" value="yes" <% if(showVMS == 1) { %>checked="true" <% } %> class="noborder" /> <fmt:message key="mvnforum.common.thread.showvms"/>
            <%--<input type="checkbox" name="AddWatchParentThread" value="yes" class="noborder" <%if (addWatchThread) {%>checked="checked"<%}%> /> <fmt:message key="mvnforum.user.viewthread.link.watch_this_thread"/>--%>
...
<div id="modal">
    ...
    <div id="content">
        <form id="uploadFormID" action="<%=urlResolver.encodeURL(request, response, "addattachmentprocess", URLResolverService.UPLOAD_URL)%>" method="post" enctype="multipart/form-data" name="submitform">
        ...
        <mvn:cssrows>
          <tr class="portlet-section-header">
            <td><fmt:message key="mvnforum.user.addattachment.prompt"/></td>
          </tr>
          <tr class="<mvn:cssrow/>">
            <td valign="top"><span class="requiredfield">*</span> <fmt:message key="mvnforum.user.addattachment.file_to_attach"/></td>
          </tr>
          <tr>
              <td>
              <div id= "file-browsers">
              <input id="my_file" type="file" name="AttachFilename"/><br/>
              <input id="PostID" type="hidden" name="PostID" value="-1"/><br/>
              <input id="offset" type="hidden" name="offset"value="0"/><br/>
              <input id="AttachDesc" type="hidden" name="AttachDesc" value=""/><br/>
          ...
          <tr>
            <td class="portlet-section-footer" align="center">
              <input type="button" name="submitbutton" value="Submit" onclick="upload();" >
            </td>
          </tr>
        </mvn:cssrows>
        </table>
...
<script type="text/javascript">
    function upload()
    {
        ...
        $("#uploadFormID").ajaxSubmit({
            type: "POST",
            url: "<%=urlResolver.encodeURL(request, response, "addattachmentprocessforThread", URLResolverService.UPLOAD_URL)%>?dt=" + new Date().getTime(),
            success: function (res) {
                message.value = message.value + '[img]' + '<% out.print(prefix); %>' + '/getattachment?attach=' + res + '[/img]';
