<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/user/listunansweredthreads.jsp,v 1.40 2008/06/13 09:23:21 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.40 $
...
function handleGo() {
  ...
  params += "status=" + document.form.status.value;
  params += "&sort=" + document.form.sort.value;
  params += "&order=" + document.form.order.value;

  gotoPage("listunansweredthreads?" + params);
}
//]]>
</script>
...
%>
<form name="form" action="<%=urlResolver.encodeURL(request, response, "listunansweredthreads", URLResolverService.ACTION_URL)%>" <mvn:method/> onsubmit="return false">
...
<table width="95%" align="center">
  <tr>
    <td nowrap="nowrap" class="portlet-font">
      <fmt:message key="mvnforum.common.filter_by.thread_status"/>
      <select name="status">
