<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/admin/viewmember.jsp,v 1.122 2008/06/23 07:53:43 lexuanttkhtn Exp $
 - $Author: lexuanttkhtn $
 - $Revision: 1.122 $
...
<table class="tborder" width="95%" align="center" cellpadding="3" cellspacing="0">
  <tr class="pagedesc">
    ...
    <a class="command" href="<%=urlResolver.encodeURL(request, response, "resetsignatureprocess?memberid=" + memberBean.getMemberID(), URLResolverService.ACTION_URL)%>"><fmt:message key="mvnforum.admin.viewmember.reset_member_signature"/></a><br/>
    <a class="command" href="<%=urlResolver.encodeURL(request, response, "resetavatarprocess?memberid=" + memberBean.getMemberID(), URLResolverService.ACTION_URL)%>"><fmt:message key="mvnforum.admin.viewmember.reset_member_avatar"/></a><br/>
    <%if (internalUserDatabase) {%>
    <a class="command" href="<%=urlResolver.encodeURL(request, response, "resetactivationprocess?memberid=" + memberBean.getMemberID(), URLResolverService.ACTION_URL)%>"><fmt:message key="mvnforum.admin.viewmember.reset_member_activation"/></a><br/>
    <%}%>
    </td>
    <td valign="top">
      <% if (watchCount > 0) { %>
      <a class="command" href="<%=urlResolver.encodeURL(request, response, "deletewatch?memberid=" + memberBean.getMemberID())%>"><fmt:message key="mvnforum.admin.viewmember.delete_all_watch"/></a> (<%=watchCount%> <% if (watchCount == 1) { %><fmt:message key="mvnforum.common.watch"/><% } else { %><fmt:message key="mvnforum.common.watches"/><% } %>)<br/>
      <% } %>
  ...
  <% } %>
    ...
    <% if (memberBean.getMemberStatus() == MemberBean.MEMBER_STATUS_ENABLE) {%>
      <% if (memberBean.getMemberID() != MVNForumConstant.MEMBER_ID_OF_ADMIN) {%>
    (<a class="command" href="<%=urlResolver.encodeURL(request, response, "changememberstatusprocess?memberid=" + memberBean.getMemberID() + "&amp;status=" + MemberBean.MEMBER_STATUS_DISABLE + "&amp;viewmember=true", URLResolverService.ACTION_URL)%>"><fmt:message key="mvnforum.admin.viewmember.disable_this_member"/></a>)
    <% } %>
    <% } else {//disabled%>
    (<a class="command" href="<%=urlResolver.encodeURL(request, response, "changememberstatusprocess?memberid=" + memberBean.getMemberID() + "&amp;status=" + MemberBean.MEMBER_STATUS_ENABLE + "&amp;viewmember=true", URLResolverService.ACTION_URL)%>"><fmt:message key="mvnforum.admin.viewmember.enable_this_member"/></a>)
    <% } %>
    </td>
  </tr>
</table>
...
<table class="tborder" width="95%" align="center" cellpadding="3" cellspacing="0">
  ...
  <tr class="<mvn:cssrow/>">
    ...
    <td>
      ...
      <%  if (memberBean.getMemberEmail().equalsIgnoreCase(memberBean.getMemberFirstEmail()) == false) {%>
