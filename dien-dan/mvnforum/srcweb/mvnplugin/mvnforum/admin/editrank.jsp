<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/admin/editrank.jsp,v 1.63 2008/06/13 09:23:18 phuongpdd Exp $
 - $Author: phuongpdd $
 - $Revision: 1.63 $
...
<mvn:head>
  <mvn:title><fmt:message key="mvnforum.common.forum.title_name"/> - <fmt:message key="mvnforum.admin.editrank.title"/></mvn:title>
...
<table width="95%" align="center">
  <tr class="nav">
    ...
    <a class="nav" href="<%=urlResolver.encodeURL(request, response, "rankmanagement")%>"><fmt:message key="mvnforum.admin.rankmanagement.title"/></a>&nbsp;&raquo;&nbsp;
    <fmt:message key="mvnforum.admin.editrank.title"/>
    </td>
  </tr>
</table>
<br/>
...
<%
RankBean rankBean = (RankBean)request.getAttribute("RankBean");
%>

<form action="<%=urlResolver.encodeURL(request, response, "editrankprocess", URLResolverService.ACTION_URL)%>" method="post" name="submitform">
<%=urlResolver.generateFormAction(request, response, "editrankprocess")%>
<input type="hidden" name="RankID" value="<%=rankBean.getRankID()%>" />
<mvn:cssrows>
<table class="tborder" width="95%" cellspacing="0" cellpadding="3" align="center">
  <tr class="portlet-section-header">
    <td colspan="2"><fmt:message key="mvnforum.admin.editrank.edit_rank_info"/>:</td>
  </tr>
  <tr class="<mvn:cssrow/>">
    <td><span class="requiredfield">*</span> <fmt:message key="mvnforum.admin.rankmanagement.minimum_posts"/>:</td>
    <td><input type="text" name="RankMinPosts" value="<%=rankBean.getRankMinPosts()%>" size="10" maxlength="6" /></td>
  </tr>
  <tr class="<mvn:cssrow/>">
    <td><span class="requiredfield">*</span> <fmt:message key="mvnforum.admin.rankmanagement.rank_title"/>:</td>
    <td><input type="text" name="RankTitle" value="<%=rankBean.getRankTitle()%>" onkeyup="initTyper(this);" size="50" /></td>
  </tr>
